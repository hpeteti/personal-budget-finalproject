...
function Visualizations() {
  ...
  useEffect(() => createPieChart(expensesPieChartRef, chartData.datasets[0].expense, chartData.labels2), [chartData]);
  useEffect(() => createDoughnutChart(budgetsDoughnutChartRef, chartData.datasets[0].budget, chartData.labels1), [chartData]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const endpoint = `https://project-mcpv.onrender.com/get-budgets/${userData}?month=${selectedMonth}`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const token = localStorage.getItem('token');
        const userData = localStorage.getItem('userData');
        const endpoint = `https://project-mcpv.onrender.com/get-expenses/${userData}?month=${selectedMonth}`;
        const res = await axios.get(endpoint, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  return (
    <main className="center" id="main" aria-label="main">
      ...
      {dataLoaded ? (
        ...
        <div>
          <p>Select a month to see visualizations of your expenses and budgets.</p>
        </div>
      )}
    </main>
  );
